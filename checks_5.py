...
import re
import requests
from veracode_api_py import Applications, Sandboxes, Scans, Findings
...
def get_sca_analysis(app_id, sandbox_id):
    """Fetch SCA analysis linked to the application and sandbox."""
    # Use the Veracode API to fetch SCA data
    url = f"{VERACODE_API_BASE}/appsec/v1/applications/{app_id}/sandboxes/{sandbox_id}/sca"
    headers = {
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch SCA data: {response.status_code} - {response.text}")
        return None

def get_api_token():
    """Get a Veracode API token using credentials from ~/.veracode/credentials."""
    from veracode_api_py.api import Credentials
...
def main():
    ...
    sca_analysis = get_sca_analysis(app_id, sandbox_id)
    if not sca_analysis:
        print("FAILED: No SCA linked with the scan.")
    else:
        print("PASSED: SCA is linked with the scan.")
        print(f"SCA Analysis Details: {sca_analysis}")
